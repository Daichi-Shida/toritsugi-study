...
_DROP_SUBSTR = (
    ...
    "思い出して", "としか思えない", "ちゃうちゃう", "無理に決まって", "実際の感想",
    "そらそう", "かと思われます",
    # 出典に混ざる下品な物言い・出題者への論評
    "ウンコ", "みたいな問題", "出題者", "でしょうか",
)
...
# 出典の話し言葉を書き言葉に置き換える表。文ごと削除すると説明が消えてしまうので、
# 意味を持つ言い回しはここで普通の文章に直し、削除は情報のない雑談だけに留める。
_COLLOQUIAL_REWRITES = (
    ("んなーこたない", "そのようなことはありません"),
    ("んなーこた、ない", "そのようなことはありません"),
    ("ってやつです", "です"),
    ("ホントに", "本当に"),
    ("ホント", "本当"),
    ("そら、", ""),
...
def _rewrite_outside_quotes(s: str) -> str:
    """引用「」の外側だけ、話し言葉を書き言葉に置き換える。"""
    out, buf, depth = [], "", 0
    def flush():
        nonlocal buf
        text = buf
        for a, b in _COLLOQUIAL_REWRITES:
            text = text.replace(a, b)
        text = re.sub(r"んです(わ|よ|ね)?(な)?。", "のです。", text)
        text = re.sub(r"んです(わ|よ|ね)?(な)?", "のです", text)
        text = re.sub(r"ますわ。", "ます。", text)
        out.append(text)
        buf = ""
    for ch in s:
        if ch == "「":
            flush(); depth += 1; out.append(ch)
        elif ch == "」":
...
def _normalize_sentence(s: str) -> str:
    ...
    # 話し言葉の言い回しを普通の文章に直す（削るのではなく書き換える）。
    # 「」内は手引きや条文の引用なので、書き換えの対象から外す。
    s = _rewrite_outside_quotes(s)
